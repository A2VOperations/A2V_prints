...
export default function CategoryClientView({ category, slug, initialProducts = [] }) {
  ...
  const isGraphicCat = Boolean(
    (category && category.serviceType === 'graphic') ||
    (initialProducts && initialProducts.some(p => p.serviceType === 'graphic')) ||
    ['logo-identity-design', 'graphic-design', 'web-design', 'digital-marketing', 'outdoor-signage', 'print-design', 'Product-Merchandize', 'Art-Illustration', 'logo-design', 'flyer-design', 'brochure-design', 'social-media-graphics', 'poster-design', 'visiting-card-design', 'letterhead-design', 'packaging-design', 'graphics-categories'].includes((category?.slug || slug || '').toLowerCase())
  );
            ...
            <>
              <div className="space-y-2 max-w-2xl relative z-10">
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 text-orange-300 text-[11px] font-extrabold uppercase tracking-wider border border-white/10">
                  📁 Custom Artwork & Brief Upload
                </span>
                <h2 className="text-xl sm:text-2xl font-black tracking-tight">
                  Have a custom requirement or artwork ready for {title}?
                </h2>
                <p className="text-slate-300 text-xs sm:text-sm leading-relaxed">
                  Upload your design files, sketch, or requirement specification directly to start your bespoke {title.toLowerCase()} order.
                </p>
              </div>
              <button
                onClick={() => setToastMessage('Select a service card below to upload your artwork and complete your order.')}
                className="shrink-0 bg-[#CC3B10] hover:bg-[#E55B2B] text-white font-extrabold px-6 py-3.5 rounded-2xl text-xs sm:text-sm transition-all shadow-lg flex items-center gap-2 group relative z-10 cursor-pointer"
              >
